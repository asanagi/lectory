...
// setupIntegrationClient initializes Firebase dependencies and returns a Connect-RPC client.
// In Staging Mode (USER_SERVICE_URL set): targets the deployed Cloud Run staging URL.
// In Local Mode (USER_SERVICE_URL empty): boots an ephemeral httptest loopback server.
func setupIntegrationClient(t *testing.T) (appv1connect.UserServiceClient, *auth.Client, *firestore.Client, string, func()) {
	t.Helper()
	...
	if credPath := os.Getenv("GOOGLE_APPLICATION_CREDENTIALS"); credPath != "" {
		app, err = firebase.NewApp(ctx, conf, option.WithCredentialsFile(credPath))
	...
	if credPath := os.Getenv("GOOGLE_APPLICATION_CREDENTIALS"); credPath != "" {
		...
		clientOpts = append(clientOpts, option.WithCredentialsFile("service-account.json"))
	}

	var firestoreClient *firestore.Client
	if dbID := os.Getenv("FIRESTORE_DATABASE"); dbID != "" && dbID != "(default)" {
		firestoreClient, err = firestore.NewClientWithDatabase(ctx, projectID, dbID, clientOpts...)
	} else if projectID != "" {
		firestoreClient, err = firestore.NewClient(ctx, projectID, clientOpts...)
	} else {
	...
	if serviceURL != "" {
		// Staging mode: point directly at deployed Cloud Run URL
		client := appv1connect.NewUserServiceClient(http.DefaultClient, serviceURL)
		cleanup := func() {
			firestoreClient.Close()
		}
		return client, authClient, firestoreClient, serviceURL, cleanup
	}

	// Local mode: boot transient loopback server with real Firestore store
...
func mintTestIDToken(t *testing.T, ctx context.Context, authClient *auth.Client, uid string) string {
	...
	if resp.StatusCode != http.StatusOK {
		...
		t.Fatalf("Identity Toolkit returned HTTP %d while exchanging test token: %s", resp.StatusCode, string(b))
	}
...
func TestUnauthenticated_401(t *testing.T) {
	client, _, _, _, cleanup := setupIntegrationClient(t)
	defer cleanup()
...
func TestAuthenticated_200_Profile(t *testing.T) {
	client, authClient, firestoreClient, _, cleanup := setupIntegrationClient(t)
	defer cleanup()
